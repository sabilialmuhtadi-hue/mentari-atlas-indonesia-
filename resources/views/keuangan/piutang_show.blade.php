...
<div class="container-fluid py-4" style="background-color: #f8fafc; min-height: 80vh;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-0 text-slate-dark fw-bold"><i class="fas fa-file-invoice-dollar text-emerald-custom me-2"></i>Detail Pembayaran & Cicilan</h1>
            <p class="text-slate-muted small mb-0 mt-1">Kelola penerimaan uang masuk dan pantau historis cicilan dari pelanggan ini.</p>
        </div>
        <a href="{{ route('keuangan.piutang.index') }}" class="btn btn-light fw-bold shadow-sm rounded-pill px-4" style="border: 1px solid #cbd5e1; color: #475569;">
    ...
    <div class="row g-4 mb-4">
        ...
        <div class="col-xl-4 col-lg-5">
            <div class="card card-custom bg-white h-100 overflow-hidden" style="border-top: 4px solid #10b981;">
                <div class="card-header bg-white py-3 border-bottom border-light">
                    <h6 class="m-0 fw-bold text-slate-dark"><i class="fas fa-info-circle text-emerald-custom me-2"></i>Rincian Tagihan</h6>
                </div>
                <div class="card-body p-4">
                    <div class="mb-4">
                        <small class="text-slate-muted fw-bold text-uppercase" style="letter-spacing: 0.5px;">No. Invoice</small>
                        <div class="fs-5 text-emerald-custom fw-bold">{{ $piutang->no_invoice ?? '-' }}</div>
                    </div>
                    <div class="mb-4">
                        <small class="text-slate-muted fw-bold text-uppercase" style="letter-spacing: 0.5px;">No. Sales Order</small>
                        <div class="fs-6 text-slate-dark"><span class="badge badge-secondary-soft rounded-pill px-3 py-1">{{ $piutang->penjualan->no_so ?? '-' }}</span></div>
                    </div>
                    <div class="mb-4">
                    ...
                    @php
                        $totalCN = $piutang->pembayarans->whereIn('metode_pembayaran', ['Credit Note', 'Retur Customer / Credit Note'])->sum('jumlah_bayar');
                        $tagihanAwalSebelumCN = $piutang->total_tagihan + $totalCN;
                    @endphp

                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="text-slate-muted fw-medium">Total Tagihan Awal:</span>
                        <span class="fw-bold text-slate-dark font-monospace-custom" style="font-size: 1.1rem;">Rp {{ number_format($tagihanAwalSebelumCN, 0, ',', '.') }}</span>
                    </div>

                    @if($totalCN > 0)
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <span class="text-slate-muted fw-medium"><i class="fas fa-tags text-warning me-1"></i> Potongan Retur / CN:</span>
                        <span class="fw-bold text-warning font-monospace-custom" style="font-size: 1.1rem;">- Rp {{ number_format($totalCN, 0, ',', '.') }}</span>
                    </div>
                    <div class="d-flex justify-content-between align-items-center mb-2 border-top border-light pt-2">
                        <span class="text-slate-muted fw-bold">Tagihan Bersih (Nett):</span>
                        <span class="fw-bold text-slate-dark font-monospace-custom" style="font-size: 1.1rem;">Rp {{ number_format($piutang->total_tagihan, 0, ',', '.') }}</span>
                    </div>
                    @endif

                    <div class="d-flex justify-content-between align-items-center mb-3 mt-2">
                        <span class="text-slate-muted fw-medium">Total Uang Masuk:</span>
                        <span class="fw-bold text-success font-monospace-custom" style="font-size: 1.1rem;">Rp {{ number_format($piutang->total_dibayar, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center p-3 rounded-3 mt-4" style="background-color: #fef2f2; border: 1px dashed #fecaca;">
                        ...
                        <span class="fw-bold text-danger font-monospace-custom fs-4">Rp {{ number_format($sisa_tagihan, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="mt-4 pt-2">
                        ...
                        @if($piutang->status_bayar == 'lunas' || $piutang->status_bayar == 'Lunas')
                        ...
                        @elseif(strtolower($piutang->status_bayar) == 'cicil')
                        ...
                        @else
                            <div class="badge badge-danger-soft text-danger fs-6 px-4 py-3 w-100 rounded-pill shadow-sm d-flex justify-content-center align-items-center">
                                <i class="fas fa-times-circle me-2 fs-5"></i> <span style="letter-spacing: 1px;">BELUM BAYAR</span>
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <div class="col-xl-8 col-lg-7">
            <div class="card card-custom bg-white h-100 overflow-hidden">
                ...
                <div class="card-body p-4">
                    @if(strtolower($piutang->status_bayar) == 'lunas' || $sisa_tagihan <= 0)
                        <div class="text-center py-5 h-100 d-flex flex-column justify-content-center align-items-center bg-emerald-soft rounded-4 border" style="border-color: #a7f3d0 !important;">
                            ...
                            <h3 class="fw-bold text-emerald-custom mb-2">Tagihan Sudah Lunas!</h3>
                            <p class="text-slate-muted mb-0">Hore! Tidak ada lagi sisa pembayaran yang perlu ditagih atau diproses untuk dokumen invoice ini.</p>
                        </div>
                    @else
                        <form action="{{ route('keuangan.piutang.bayar', $piutang->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="row g-4 mb-4">
                                <div class="col-md-6">
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="keterangan" class="form-control form-control-custom shadow-sm" rows="2" placeholder="Contoh: Pembayaran tahap pertama, uang dititip lewat supir pengiriman, dll."></textarea>
                            </div>
                            
                            <div class="mb-4 p-3 rounded-3" style="background-color: #f8fafc; border: 1px dashed #cbd5e1;">
                                <label class="form-label fw-bold text-slate-dark small"><i class="fas fa-camera me-1 text-slate-muted"></i> Upload Bukti Bayar / Struk <span class="text-slate-muted fw-normal">(Opsional)</span></label>
                            ...
                            <div class="text-end pt-2">
                                <button type="submit" class="btn btn-emerald-custom btn-lg px-5 shadow-sm rounded-pill w-100 w-md-auto" onclick="return confirm('Konfirmasi: Proses input pembayaran ini? Pastikan uang fisik / transfer sudah Anda terima dengan valid.')">
                                    <i class="fas fa-save me-2"></i> Simpan Pembayaran Masuk
                                </button>
                            </div>
                        </form>
                    @endif
    ...
    <div class="card card-custom bg-white overflow-hidden mb-4">
        <div class="card-header bg-white py-3 border-bottom border-light">
            <h6 class="m-0 fw-bold text-slate-dark"><i class="fas fa-history text-emerald-custom me-2"></i>Riwayat Arus Kas / Cicilan Pembayaran</h6>
        </div>
                ...
                <table class="table table-hover align-middle mb-0" style="font-size: 0.85rem; width: 100%;">
                    <thead class="table-custom-header">
                        <tr>
                            ...
                            <th class="text-end" width="15%">Nominal Uang Masuk</th>
                            <th class="text-center" width="15%">Metode Bayar</th>
                            <th width="15%">Penerima (Admin)</th>
                            <th class="pe-4">Catatan & Bukti</th>
                        </tr>
                    </thead>
                    <tbody>
                        @php $noKas = 1; @endphp
                        @forelse($piutang->pembayarans->whereNotIn('metode_pembayaran', ['Credit Note', 'Retur Customer / Credit Note'])->sortBy('created_at') as $bayar)
                        <tr>
                            ...
                            <td>
                                <span class="fw-semibold text-slate-dark d-block"><i class="fas fa-user-circle text-emerald-custom me-1"></i> {{ $bayar->penerima->name ?? 'Sistem' }}</span>
                            </td>
                            <td class="pe-4">
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-5 text-slate-muted bg-white">
                                <div class="d-flex flex-column align-items-center">
                                    ...
                                    <span>Belum ada riwayat penerimaan uang kas / transfer untuk tagihan ini.</span>
                                </div>
                            </td>
                        </tr>
                        @endforelse
                    ...
                    <tfoot class="table-light">
                        <tr>
                            <td colspan="2" class="text-end fw-bold text-slate-dark pt-3 pb-3">TOTAL KAS MASUK:</td>
                            <td class="text-end text-success fw-bold font-monospace-custom fs-6 pt-3 pb-3">
                                Rp {{ number_format($piutang->pembayarans->whereNotIn('metode_pembayaran', ['Credit Note', 'Retur Customer / Credit Note'])->sum('jumlah_bayar'), 0, ',', '.') }}
                            </td>
                            <td colspan="3"></td>
                        </tr>
                    </tfoot>
    ...
    {{-- TABEL ADJUSMENT: Riwayat Pemotongan Khusus Credit Note (CN) --}}
    <div class="card card-custom bg-white overflow-hidden" style="border-top: 3px solid #f59e0b;">
        <div class="card-header bg-white py-3 border-bottom border-light">
            <h6 class="m-0 fw-bold text-slate-dark"><i class="fas fa-tags text-warning me-2"></i>Riwayat Penyesuaian / Potongan Credit Note (CN)</h6>
        </div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0" style="font-size: 0.85rem; width: 100%;">
                    <thead class="table-custom-header">
                        <tr>
                            ...
                            <th class="text-center" width="15%">Label Pemotongan</th>
                    ...
                    <tbody>
                        @php $noCN = 1; @endphp
                        @forelse($piutang->pembayarans->whereIn('metode_pembayaran', ['Credit Note', 'Retur Customer / Credit Note'])->sortBy('created_at') as $cn)
                        <tr>
                            <td class="ps-4 text-center fw-bold text-slate-muted">{{ $noCN++ }}</td>
                            <td>
                                <span class="fw-bold text-slate-dark d-block mb-1">{{ \Carbon\Carbon::parse($cn->tanggal_bayar)->format('d M Y') }}</span>
                                <small class="text-slate-muted"><i class="far fa-clock me-1"></i> {{ \Carbon\Carbon::parse($cn->tanggal_bayar)->format('H:i') }} WIB</small>
                            </td>
                            <td class="text-center">
                                <span class="badge badge-warning-soft text-dark fw-bold rounded-pill px-3 py-1.5" style="color: #92400e !important; border-color: #fcd34d;">
                                    <i class="fas fa-cut me-1"></i> {{ $cn->metode_pembayaran }}
                                </span>
                            </td>
                            <td class="text-end fw-bold text-warning font-monospace-custom" style="font-size: 0.95rem;">
                                - Rp {{ number_format($cn->jumlah_bayar, 0, ',', '.') }}
                            </td>
                            <td>
                                <span class="fw-semibold text-slate-dark d-block"><i class="fas fa-user-shield text-warning me-1"></i> {{ $cn->penerima->name ?? 'Sistem' }}</span>
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="text-center py-5 text-slate-muted bg-white">
                                <div class="d-flex flex-column align-items-center">
                                    ...
                                    <span>Belum ada klaim potongan Credit Note (CN) atau retur pada invoice ini.</span>
                                </div>
                            </td>
                        </tr>
                        @endforelse
                    ...
                    <tfoot class="table-light">
                        <tr>
                            <td colspan="3" class="text-end fw-bold text-slate-dark pt-3 pb-3">TOTAL POTONGAN CN:</td>
                            <td class="text-end text-warning fw-bold font-monospace-custom fs-6 pt-3 pb-3">
                                - Rp {{ number_format($piutang->pembayarans->whereIn('metode_pembayaran', ['Credit Note', 'Retur Customer / Credit Note'])->sum('jumlah_bayar'), 0, ',', '.') }}
                            </td>
                            <td colspan="2"></td>
                        </tr>
                    </tfoot>
